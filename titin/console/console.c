...
static inline void vga_out(uint16_t port, uint8_t value)
{
    __asm__ volatile (
        "outb %0, %1"
        :
...
static void vga_cursor_move(uint8_t x, uint8_t y)
{
    uint16_t position =
        (uint16_t)y * SCREEN_WIDTH + x;

    vga_out(VGA_INDEX_PORT, 0x0F);
    vga_out(VGA_DATA_PORT, position & 0xFF);

    vga_out(VGA_INDEX_PORT, 0x0E);
    vga_out(VGA_DATA_PORT, (position >> 8) & 0xFF);
}

static void console_scroll(void)
{
    for (uint64_t y = 1; y < SCREEN_HEIGHT; y++) {
    ...
    for (uint64_t x = 0; x < SCREEN_WIDTH; x++) {
        video[
            (SCREEN_HEIGHT - 1) * SCREEN_WIDTH + x
        ] = (uint16_t)' ' | 0x0700;
    }
...
void console_cursor_set(uint8_t x, uint8_t y)
{
    ...
    vga_cursor_move(cursor_x, cursor_y);
}
